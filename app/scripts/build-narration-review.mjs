...
const SCREENS = [
  ...
  },
  {
    ...
    frameGroups: [
      ...
      {
        title: 'Arabic — part two (4.2) drives these three',
        only: 'ar',
        frames: [
          { src: 'visit-step-sleepy', label: 'sleepy juice', at: 'holds the first 9 s of 4.2, while it explains the juice' },
          { src: 'visit-step-count', label: 'count', at: '+9.1 s — eyes shut, a beat before "واحد"' },
          { src: 'visit-step-count-ten', label: 'count to ten', at: '+25.4 s — on "عشرة"' },
          { src: 'visit-step-clean', label: 'clean', at: 'after 4.2 ends, on visit.step.clean' },
        ],
      },
      {
        title: 'English — the same seven, one spoken line per frame, 900 ms between',
        only: 'en',
        frames: [
          ...
          { src: 'visit-step-count', label: 'count', at: 'eyes shut' },
          { src: 'visit-step-count-ten', label: 'count to ten', at: '+17.0 s — on "ten"' },
          { src: 'visit-step-clean', label: 'clean' },
        ],
      },
    ],
    blurb:
      'The walk-through. Both languages now run the same seven steps in the same order — English only had four until 2026-09-05, skipping the counting entirely. How they are voiced still differs: English speaks each step separately with a 900 ms pause, while Arabic reaches the first four through one 35.5 s recording, moving the picture on timed cues underneath it.',
    events: [
      ...
      { kind: 'line', id: 'visit.simulation', only: 'ar', label: 'Part one (4.1). Picture cues at 7.54 s, 15.74 s, 20.18 s, 28.74 s; ends with the sleepy juice on screen' },
      { kind: 'line', id: 'visit.juiceCount', only: 'ar', label: 'Part two (4.2). The juice picture holds for its first 9 s, then eyes close for the count; word timings transcribed from the recording' },
      ...
      { kind: 'gap', ms: 900, only: 'en' },
      { kind: 'line', id: 'visit.step.count', only: 'en', label: 'Eyes shut. English gained this on 2026-09-05' },
      { kind: 'line', id: 'visit.countToTen', only: 'en', label: 'Built number by number with 950 ms gaps to count into (scripts/make-english-count.mjs); the ten fingers arrive at +17.0 s, on the last number' },
      { kind: 'note', only: 'ar', text: 'Arabic reaches both of these inside 4.2 instead, so ar/visit.step.count.mp3 and ar/visit.countToTen.mp3 are no longer played — two recordings, 336 KB, still shipped and precached. Worth removing once the new cut is signed off.' },
