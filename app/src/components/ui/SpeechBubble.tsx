...
export function SpeechBubble({ stringId }: { stringId: StringId }) {
  ...
  return (
    <motion.div
      ...
      className="relative bg-white rounded-3xl px-5 py-3 text-xl font-bold shadow-md max-w-[85vw] cursor-pointer"
    >
      {t(lang, stringId, { name: childName })}
      <span className="absolute -bottom-2 start-8 w-4 h-4 bg-white rotate-45 rounded-sm" />
    </motion.div>
  )
}
